...
<div class="container">
  ...
  gpa.common.DbUtil util = new gpa.common.DbUtil();
   Connection con  = java.sql.DriverManager.getConnection(util.url+"?useUnicode=true" + 
"&characterEncoding=gbk&user="+util.user+"&password="+util.password); 
Statement  stmt=con.createStatement(); 
String sql = "select * from gpainfo where 1=1"; 

ResultSet  rst=stmt.executeQuery(sql); 

gpa.onlineSubmit.OnlineBean ob = new gpa.onlineSubmit.OnlineBean();
...
<tr>
  ...
  <td><select id="sel" name="FSTDAY_CHECK<%=i%>" ><option selected="selected"><%=rst.getString("FSTDAY_CHECK") %></option><option  value="1">1</option><option value="0">0</option></select></td>
  <td><select id="sel" name="FSTDAY_SCORE<%=i%>" ><option selected="selected"><%=rst.getString("FSTDAY_SCORE") %></option><option  value="A+">A+</option><option value="A">A</option><option value="A-">A-</option><option value="B+">B+</option><option value="B">B</option><option value="B-">B-</option><option value='C+'>C+</option><option value='C'>C</option><option value='F'>F</option></select></td>
   <td><select id="sel" name="SECTDAY_CHECK<%=i%>" ><option selected="selected"><%=rst.getString("SECTDAY_CHECK") %></option><option  value="1">1</option><option value="0">0</option></select></td>
  <td><select id="sel" name="SECDAY_SCORE<%=i%>" ><option selected="selected"><%=rst.getString("SECDAY_SCORE") %></option><option  value="A+">A+</option><option value="A">A</option><option value="A-">A-</option><option value="B+">B+</option><option value="B">B</option><option value="B-">B-</option><option value='C+'>C+</option><option value='C'>C</option><option value='F'>F</option></select></td>
   <td><select id="sel" name="THRDAY_CHECK<%=i%>" ><option selected="selected"><%=rst.getString("THRDAY_CHECK") %></option><option  value="1">1</option><option value="0">0</option></select></td>
...
%> 
     ...
     <p><button id="download" class="btn" type="submit" >提交</button></p>
 </form>
</div>
</html>
